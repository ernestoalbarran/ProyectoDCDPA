...
namespace Datos
{
    public class DatoFirmante1 
    {
        ...
        public int InsertFirmante1( string RFC_PEC, string RFC_CONSEJERO1, string RFC_CONSEJERO2, string RFC_CONSEJERO3, string RFC_CONSEJERO4, string RFC_CONSEJERO5,
...
string RFC_CONSEJERO6, string SIGLAS, DateTime FECHA_OFICIO, int ID_PLANTEL, int ID_AREA, int ID_PERIODO)
        {
            ...
            return ejecutaNonQuery("RELACION_FP", parametros);
        }
            ...
            using (DbConnection con = dpf.CreateConnection())
            {
                ...
                using (DbCommand cmd = dpf.CreateCommand())
                {
                    ...
                    using (DbDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            LstFirmante1.Add(new Firmante1((int)dr["ID_FIRMANTE1"], (string)dr["RFC_PEC"], (string)dr["RFC_CONSEJERO1"],
                                ...
                                (string)dr["SIGLAS"], (DateTime)dr["FECHA_OFICIO"]));
                        }
        ...
        public DataTable RecuperaDTFirmantes(String idArea, String idPlantel)
        {
            ...
            DbParameter paramidArea = dpf.CreateParameter();
            paramidArea.Value = idArea;
            paramidArea.ParameterName = "idArea";
            parametros.Add(paramidArea);

            DbParameter paramidPlantel = dpf.CreateParameter();
            paramidPlantel.Value = idPlantel;
            paramidPlantel.ParameterName = "idPlantel";
            parametros.Add(paramidPlantel);

            DataTable FirmantesDT = new DataTable();
            DataColumn Col1 = new DataColumn("RFC");
            DataColumn Col2 = new DataColumn("NOMBRE");
            Col1.DataType = System.Type.GetType("System.String");
            Col2.DataType = System.Type.GetType("System.String");
            FirmantesDT.Columns.Add(Col1);
            FirmantesDT.Columns.Add(Col2);
            
            string StoredProcedure = "SP_OBTENER_FIRMANTE1";
            using (DbConnection con = dpf.CreateConnection())
            {
                con.ConnectionString = constr;
                using (DbCommand cmd = dpf.CreateCommand())
                {
                    ...
                    cmd.CommandText = StoredProcedure;
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add(paramidArea);
                    cmd.Parameters.Add(paramidPlantel);
                    con.Open();
                    using (DbDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            DataRow row = FirmantesDT.NewRow();
                            row[Col1] = (string)dr["RFC"];
                            row[Col2] = (string)dr["NOMBRE"];
                            FirmantesDT.Rows.Add(row);
                        }
